...
if platform?('windows')
  displayversion = node['jenkins']['server']['win_displayversion']
  version = node['jenkins']['server']['version']

  cache = Chef::Config[:file_cache_path]

  windows_zipfile "#{cache}" do
    source "#{node['jenkins']['mirror']}/windows/jenkins-#{version}.zip"
    action :unzip
    overwrite true
    not_if {::File.exists?("#{cache}/jenkins-#{version}.msi")}
  end

  # The Jenkins full installer for Windows sets it up as a service automatically
  windows_package "Jenkins #{version}" do
    source "#{cache}/jenkins-#{version}.msi"
    installer_type :msi
    action :install
...
# Original super-UNIX/Linux-specific stuff follows...
